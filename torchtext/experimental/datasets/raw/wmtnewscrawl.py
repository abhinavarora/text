...
_AVAILABLE_LANGUAGES = ["cs", "en", "fr", "es", "de"]
...
def WMTNewsCrawl(root, split, year=2010, language="en"):
    ...
    path = _download_extract_validate(
        root, URL, MD5, _PATH, _EXTRACTED_FILES[language], _EXTRACTED_FILES_MD5[language], hash_type="md5"
    )
    logging.info("Creating {} data".format(split))
    return _RawTextIterableDataset(DATASET_NAME, NUM_LINES[split], _read_text_iterator(path))
